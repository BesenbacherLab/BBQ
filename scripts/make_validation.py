    ...
    if L[6] == 'PASS' or L[6] == 'lowBQ':
        ...
        for i in range(len(oldBQs)):
            ...
            if kmer2pattern is None:
                counter[(muttype, nonsym_muttype, oldBQs[i], max_oldBQ, newBQs[i], validation, D['n_mismatch'], D['N_A_37'], readNum[i])] += 1
            else:
                if kmer not in kmer2pattern[max_oldBQ][muttype]:
                    kmer = reverse_complement(kmer)
                pattern = kmer2pattern[max_oldBQ][muttype][kmer]
                counter[(muttype, nonsym_muttype, oldBQs[i], max_oldBQ, newBQs[i], validation, D['n_mismatch'], D['N_A_37'], readNum[i], pattern)] += 1
