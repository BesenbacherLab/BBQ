...
def get_alleles_w_probabities_update(pileupcolumn, ref, ref_kmer, correction_factor, prior_N, no_update, double_adjustment=2, filter_reads = True):
    ...
    # calculate error rates for single reads:
    ...
    for A in seen_alt:
        for overlap_type, X, read_BQ, read_MQ, enddist, has_indel, has_clip, NM in events[A]:
            ...
            posterior_from_A = new_correction_factor[read_BQ][muttype_from_A][kmer_from_A]
            posterior_from_R = new_correction_factor[read_BQ][muttype_from_R][kmer_from_R]
...
def get_alleles_w_probabities_update_joint(pileupcolumn, ref, ref_kmer, correction_factor, prior_N, no_update, double_adjustment=2, filter_reads = True):
    """
    Returns a dictionary that maps from allele A to a list of tuples with probability of 
    observing Alt allele A given then read and probability of observing ref allele R given 
    the read. I.e.: base_probs[A] = [(P(A -> X_read_i|read_i),P(R -> X_read_i|read_i), ..., ]
